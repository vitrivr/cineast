...
public class ExtractionPipeline implements Runnable, ExecutionTimeCounter {
  ...
  /**
   * Sets the running flag to false which will halt the execution of the ExtractionPipeline as soon as possible. Even when halted, there might still be ExtractionTasks in the executor that have not finished yet!
   */
  ...
  /**
   * Can be used to emit a SegmentContainer into the ExtractionPipeline. Invoking this method will add the container to the local queue.
   * <p>
   * If that queue is full, this method will block for the specified amount of time or until space to becomes available. If during that time, no space becomes available, the method returns false. This is an indication that emission of the segment should be retried later.
   *
