...
public class ContinuousQueryDispatcher {
  ...
  private static final Cache<Triple<AbstractQueryTermContainer, TObjectDoubleHashMap<Retriever>, CacheableQueryConfig>, List<SegmentScoreElement>> queryCache = QUERY_CACHE_ENABLED ? CacheBuilder.newBuilder().maximumSize(QUERY_CACHE_SIZE).expireAfterWrite(QUERY_CACHE_LIFE, TimeUnit.SECONDS).build() : null;

  private static final Cache<Triple<String, TObjectDoubleHashMap<Retriever>, CacheableQueryConfig>, List<SegmentScoreElement>> segmentIdCache = QUERY_CACHE_ENABLED ? CacheBuilder.newBuilder().maximumSize(QUERY_CACHE_SIZE).expireAfterWrite(QUERY_CACHE_LIFE, TimeUnit.SECONDS).build() : null;
