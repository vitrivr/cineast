...
public class TagImportHandler extends DataImportHandler {
  ...
  public void doImport(Path root) {
    try {
      ...
      this.futures.add(this.service.submit(new DataImportRunner(new TagImporter(root.resolve("tagNames.tsv"), TagReader.TAG_ID_COLUMNNAME, TagReader.TAG_NAME_COLUMNNAME, null, TagReader.TAG_DESCRIPTION_COLUMNNAME), TagReader.TAG_ENTITY_NAME, "tags")));
      this.futures.add(this.service.submit(new DataImportRunner(new TagImporter(root.resolve("uniqueExpandedTagInstances.tsv"), "id", "tagid", "score"), SegmentTags.SEGMENT_TAGS_TABLE_NAME, "unique mappings from segments and tags")));
