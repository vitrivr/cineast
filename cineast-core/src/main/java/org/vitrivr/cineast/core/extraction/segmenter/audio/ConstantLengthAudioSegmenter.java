...
/**
 * Merges multiple AudioFrames into a single AudioSegment using a constant number of frames per AudioSegment. The length of an AudioSegment in frames AND the overlapfactor between two subsequent AudioSegments can be defined upon onstruction of the ConstantLengthAudioSegmenter.
 *
 * @see AudioSegment
 * @see AudioFrame
 */
public class ConstantLengthAudioSegmenter implements Segmenter<AudioFrame> {
  ...
  /**
   * Constructor for {@link ConstantLengthAudioSegmenter required for instantiation through {@link ReflectionHelper }.
   *
  ...
  /**
   * Returns the next SegmentContainer from the source OR null if there are no more segments in the outputQueue. As generation of SegmentContainers can take some time (depending on the media-type), a null return does not necessarily mean that the Segmenter is done segmenting. Use the complete() method to check this.
   * <p>
   * Important: This method should be designed to block and wait for an appropriate amount of time if the Segmenter is not yet ready to deliver another segment! It's up to the Segmenter how long that timeout should last.
   */
  @Override
  ...
  /**
   * Closes the Segmenter. This method should cleanup and relinquish all resources. Especially, calling this method should also close the Decoder associated with this Segmenter instance.
   * <p>
  ...
  /**
   * This methods pulls AudioFrames as they become available from the Decoder and adds them the the Deque of AudioFrames. Once that queue holds enough AudioFrames it is drained and a AudioSegment is emmitted.
   * <p>
   * The overlapfactor defines how many frames re-enter the queue once they have been added to the new Segment.
   *
   * @see Thread#run()
   */
  @Override
