...
public class SingleFrameOCRSearch extends AbstractTextRetriever {
  ...
  class TesseractRecognizer {
    private final Logger LOGGER = Logger.getLogger( AttentionOCRrecognizer.class.getName() );
  ...
  class AttentionOCRrecognizer{
    ...
    private String executeAOCRGraph(byte[] graphDef, Tensor image) {
  ...
  class SceneTextOCR{
    private final Logger LOGGER = Logger.getLogger( SceneTextOCR.class.getName() );
...
//        Imgproc.GaussianBlur(processedImg, processedImg, new Size(3, 3),0);
      Imgproc.adaptiveThreshold(processedImg, processedImg, 255, Imgproc.ADAPTIVE_THRESH_MEAN_C ,Imgproc.THRESH_BINARY_INV, 11, 4);
    ...
    private List<RotatedRect> decode(Mat scores, Mat geometry, List<Float> confidences, float scoreThresh) {
      int W = geometry.cols();
