 ...
 * <p>
 * { image: <Base 64 encoded image>, map: List of objects mapping the class to the color (as used in the image. }
 */
public class SemanticMapQueryTermContainer extends AbstractQueryTermContainer {
  ...
  public SemanticMapQueryTermContainer(JsonNode jsonNode) {
    if (!jsonNode.has(IMAGE_FIELD_NAME)) {
      throw new IllegalArgumentException("The provided data structure does not contain the required field 'image' (semantic map).");
    }
    if (!jsonNode.has(MAP_FIELD_NAME)) {
      throw new IllegalArgumentException("The provided data structure does not contain the required field 'map' (category to color map).");
    }
