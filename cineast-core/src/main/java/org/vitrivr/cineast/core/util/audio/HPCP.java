...
/**
 * This class can be used to calculate the Harmonic Pitch Class Profile (HPCP) of a Short-Term Fourier Transform or a single Spectrum using the approach outlined by E. Gomez in [1].
 * <p>
 ...
 * <p>
 * [2] Kurth F. & Müller M. (2008). Efficient Index-Based Audio Matching. IEEE TRANSACTIONS ON AUDIO, SPEECH, AND LANGUAGE PROCESSING
 *
 * @see STFT
 * @see Spectrum
 */
public class HPCP {
  ...
  /**
   * Adds the contribution of a STFT to the HPCP by adding the contributions all the power spectra of that STFT to the HPCP.
   *
  ...
  /**
   * Adds the contribution of a single spectrum to the HPCP. The spectrum can either be a power or magnitude spectrum.
   *
  ...
  /**
   * Returns the HPCP vectorwhich has been normalized by the sum of its components as proposed in [2].
   *
   * @param idx Zero based index of the time-frame for which to return the HPCP.
  ...
  /**
   * Returns the HPCP vector which has been normalized by the value of its maximum component as proposed in the original paper ([1]).
   *
   * @param idx Zero based index of the time-frame for which to return the HPCP.
  ...
  /**
   * Returns the mean HPCP array (i.e. the HPCP arithmetic mean of all HPCP contributions).
   *
  ...
  /**
   * Calculates the contribution of a given frequency (usually from a spectrum) to a given bin in the HPCP.
   *
