 ...
 * <p>
 * Currently, one AudioFrame holds an arbitrary number of samples and channels (as returned by the respective decoder). An audio-segment groups multiple such frames and the methods defined in this interface facilitate easy access to the underlying data.
 */
public interface AudioFrameProvider {
  ...
  /**
   * Returns the total duration  in seconds of all samples in the frames segment (i.e. across all frames).
   *
  ...
  /**
   * Returns the samplingrate of the segment. That rate usually determined by the first AudioFrame added to the segment and must be the same for all frames.
   *
  ...
  /**
   * Returns the number of channels for the frames segment. Is usually determined by the first AudioFrame added to the segment and must be the same for all frames.
   *
