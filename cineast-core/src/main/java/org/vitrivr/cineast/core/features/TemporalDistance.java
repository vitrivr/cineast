...
/**
 * A feature that calculates similarity based on the distance between the timestamps of two objects measured in seconds.
 *
 ...
 * it uses a hyperbolic correspondence function scaled to days, i.e. 13h20min correspond to 90% similarity, 5 days to 50% and 45 days to 10%. This correspondence is likely unfit for many datasets and needs to be adjusted accordingly.
 */
public class TemporalDistance extends MetadataFeatureModule<InstantVector> {
  ...
  /**
   * Extracts the timestamp from the specified path using Exif GPS data. If not present, it retrieves the coordinates from a complementary JSON file named after the original document. See {@link GpsData} for more information.
   *
   * @param object Path to the file for which the coordinates should be extracted.
   * @return an {@link Optional} containing an {@link InstantVector} based on the extracted Exif or JSON timestamp, if found, otherwise an empty {@code Optional}.
   */
  @Override
