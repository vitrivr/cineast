...
public class SceneLights {
    ...
    public SceneLights(Supplier<Vector3f> position, int point) {
        // Default settings as no lights are set
        this.ambientLight = new AmbientLight(LightColor.WHITE, 0.0F);

        this.directionalLight = new DirectionalLight(LightColor.WHITE, position, 0.1f);

        this.pointLights = new ArrayList<>();
        this.spotLights = new ArrayList<>();
        for (var ic = 0; ic < point; ++ic) {
            this.pointLights.add(new PointLight(LightColor.RED.getRGB(), position.get().rotateY((float) ic * (float) Math.PI / (float) point ), 0.1f / (point)));
        }
    }
