...
/**
 * Classes implementing this interface are intended to generate ID's for multimedia-objects. The classes should be designed in such a way that:
 * <p>
 * - The same instance of the class can be re-used (e.g. for an extraction run). - Two invocations of next() generate different ID's, unless path & type are identical for both invocations AND the class intends to generate ID's specific for the combination of these values.
 */
