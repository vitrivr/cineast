...
public class WeightedChiSquaredDistance extends ChiSquaredDistance {

  private static final long serialVersionUID = -2828977206455614937L;
  private final float[] weights;

  WeightedChiSquaredDistance(float[] weights) {
    if (weights == null) {
  ...
  public double applyAsDouble(float[] t, float[] u) {
    ...
    for (int i = 0; i < len; ++i) {
      dist += (((t[i] - u[i]) * (t[i] - u[i])) / (t[i] + u[i])) * this.weights[i];
    }
