...
public class NeighbouringQueryMessageHandler extends AbstractQueryMessageHandler<NeighboringSegmentQuery> {
    ...
    /**
     * Handles a {@link NeighboringSegmentQuery} message. Makes a lookup for the {@link SegmentDescriptor}s that are
    ...
    public void handle(Session session, NeighboringSegmentQuery message) {
        ...
        this.write(session, new SegmentQueryResult(uuid, this.loadSegments(message.getSegmentIds())));
