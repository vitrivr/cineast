...
public abstract class AbstractFeatureModule implements Extractor, Retriever {
  ...
  protected void persist(String shotId, List<ReadableFloatVector> fvs) {
    List<SimpleFeatureDescriptor> entities = fvs.stream()
        .map(fv -> new SimpleFeatureDescriptor(shotId, fv)).collect(Collectors.toList());
  ...
  protected List<StringDoublePair> getSimilar(float[] vector, ReadableQueryConfig qc) {
    ...
    List<StringDoublePair> distances = this.selector.getNearestNeighbours(
        Config.sharedConfig().getRetriever().getMaxResultsPerModule(), vector, "feature", qcc);
    if (distances == null) {
      return new ArrayList<>(1);
    }
