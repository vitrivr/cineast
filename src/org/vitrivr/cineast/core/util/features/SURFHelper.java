package org.vitrivr.cineast.core.util.features;
...
/**
 * Extracts features based on the SURF algorithm described in [1]. For feature detection, the Fast Hession (FH-9) algorithm is used.
 *
...
public class SURFHelper {
     ...
     * which yields less features but operates a bit faster.
     *
     * @param image Image for which to obtain the SURF descriptors.
     * @return
     */
    public static DetectDescribePoint<GrayF32, BrightFeature> getFastSurf(BufferedImage image) {
         /* Obtain raw SURF descriptors using the configuration above (FH-9 according to [1]). */
        GrayF32 gray = ConvertBufferedImage.convertFromSingle(image, null, GrayF32.class);
        DetectDescribePoint<GrayF32, BrightFeature> surf = FactoryDetectDescribe.surfFast(FAST_HESSIAN, null, null, GrayF32.class);
        surf.detect(gray);
        return surf;
    }
}
