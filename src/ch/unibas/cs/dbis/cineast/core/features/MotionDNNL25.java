...
@Deprecated
public class MotionDNNL25 implements Retriever {

	private DBSelector selector;
	private static final double maxDist = 139075d / 2d;
	...
	public List<LongDoublePair> getSimilar(long shotId) {
		...
		ResultSet rset = this.selector.select("WITH q AS (SELECT vector FROM features.motiondnnl25 WHERE id = " + shotId + ") SELECT id FROM features.motiondnnl25, q USING DISTANCE MINKOWSKI(2)(q.vector, motiondnnl25.vector) ORDER USING DISTANCE LIMIT " + limit);
