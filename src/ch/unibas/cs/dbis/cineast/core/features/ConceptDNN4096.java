...
@Deprecated
public class ConceptDNN4096 implements Retriever {

	private DBSelector selector;
	private static final double maxDist = 2d; //TODO adjust
	...
	public List<LongDoublePair> getSimilar(long shotId) {
		...
		ResultSet rset = this.selector.select("WITH q AS (SELECT vector FROM features.conceptdnn4096 WHERE id = " + shotId + ") SELECT id FROM features.conceptdnn4096, q USING DISTANCE MINKOWSKI(2)(q.vector, conceptdnn4096.vector) ORDER USING DISTANCE LIMIT " + limit);
