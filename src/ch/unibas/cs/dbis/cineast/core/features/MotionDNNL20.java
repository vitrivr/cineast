...
@Deprecated
public class MotionDNNL20 implements Retriever {

	private DBSelector selector;
	private static final double maxDist = 1108952d / 2d;
	...
	public List<LongDoublePair> getSimilar(long shotId) {
		...
		ResultSet rset = this.selector.select("WITH q AS (SELECT vector FROM features.motiondnnl20 WHERE id = " + shotId + ") SELECT id FROM features.motiondnnl20, q USING DISTANCE MINKOWSKI(2)(q.vector, motiondnnl20.vector) ORDER USING DISTANCE LIMIT " + limit);
